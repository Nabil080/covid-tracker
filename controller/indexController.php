...
function getChosenData(
    ...
    foreach ($covid_data[$data_source] as $key) {
        $data_items = [];
        if ($date) {
            $data_items['Date'] = substr($key['Date'], 0, strpos($key['Date'], 'T'));
        }
        if ($pays) {
            $data_items['Pays'] = $key['Pays'];
        }
        if ($infection) {
            $data_items['Infection'] = $key['Infection'];
        }
        if ($deces) {
            $data_items['Deces'] = $key['Deces'];
        }
        if ($taux_deces) {
            $data_items['TauxDeces'] = $key['TauxDeces'];
        }
...
{
    $filtered_data = [];
    if (is_string($pays_filter)) {
        $filter_array = explode(',', $pays_filter);
    } else {
        $filter_array = $pays_filter;
    }
